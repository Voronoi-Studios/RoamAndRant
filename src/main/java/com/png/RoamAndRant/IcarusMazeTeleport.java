...
public class IcarusMazeTeleport extends SimpleInstantInteraction {
    @Nonnull
    private static final HytaleLogger LOGGER = HytaleLogger.forEnclosingClass();
    @Nonnull
    public static final BuilderCodec<IcarusMazeTeleport> CODEC = BuilderCodec.builder(
                    IcarusMazeTeleport.class, IcarusMazeTeleport::new, SimpleInstantInteraction.CODEC
            )
            ...
            .add()
            .append(new KeyedCodec<>("NorthPosition", Vector3d.CODEC), (o, i) -> o.northPosition = i, o -> o.northPosition)
            .add()
            .append(new KeyedCodec<>("EastPosition", Vector3d.CODEC), (o, i) -> o.eastPosition = i, o -> o.eastPosition)
            .add()
            .append(new KeyedCodec<>("SouthPosition", Vector3d.CODEC), (o, i) -> o.southPosition = i, o -> o.southPosition)
            .add()
            .append(new KeyedCodec<>("WestPosition", Vector3d.CODEC), (o, i) -> o.westPosition = i, o -> o.westPosition)
            .add()
            .build();
    ...
    private Vector3d northPosition;
    private Vector3d eastPosition;
    private Vector3d southPosition;
    private Vector3d westPosition;
    ...
    protected void firstRun(@Nonnull InteractionType type, @Nonnull InteractionContext context, @Nonnull CooldownHandler cooldownHandler) {
        ...
        if (playerComponent != null && !playerComponent.isWaitingForClientReady()) {
            ...
            TransformComponent transformComponent = playerComponent.getReference().getStore().getComponent(playerComponent.getReference(), TransformComponent.getComponentType());

            Transform spawnPoint = new Transform();
            Vector3d dir = transformComponent.getPosition().normalize();
            if (Math.abs(dir.x) > Math.abs(dir.z)) {
                if (dir.x > 0) {
                    spawnPoint.setPosition(eastPosition);
                } else {
                    spawnPoint.setPosition(westPosition);
                }
            } else {
                if (dir.z > 0) {
                    spawnPoint.setPosition(southPosition);
                } else {
                    spawnPoint.setPosition(northPosition);
                }
            }
            ...
            if (!archetype.contains(Teleport.getComponentType()) && !archetype.contains(PendingTeleport.getComponentType())) {
                ...
                if (targetWorld != null) {
                    teleportToLoadedWorld(ref, commandBuffer, targetWorld, playerComponent, spawnPoint);
                } else {
                    ...
                    teleportToLoadingWorld(ref, commandBuffer, worldFuture, currentWorld, playerComponent, spawnPoint);
                }
            }
        }
    }
...
//                                }
                            ...
                            .whenComplete((ret, ex) -> {
                                ...
                                //All this can probably be deleted, check when you have time
                                if (ret == null) {
